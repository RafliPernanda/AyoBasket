...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($id_lapangan) || empty($nama_pemesan) || empty($tanggal_main) || empty($jam_mulai) || empty($jam_selesai) || empty($status_reservasi)) {
        ...
        try {
            $sql = "UPDATE reservasi SET id_lapangan=?, nama_pemesan=?, no_telepon=?, tanggal_main=?, jam_mulai=?, jam_selesai=?, status_reservasi=? WHERE id_reservasi=?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$id_lapangan, $nama_pemesan, $no_telepon, $tanggal_main, $jam_mulai, $jam_selesai, $status_reservasi, $id]);
            header("Location: index.php");
            exit;
...
// Fetch existing data
try {
    $stmt = $pdo->prepare("SELECT * FROM reservasi WHERE id_reservasi = ?");
    $stmt->execute([$id]);
...
<div class="card" style="margin-top: 2rem;">
    <div class="card-header">
        <h2>Edit Reservasi #<?= htmlspecialchars($reservasi['id_reservasi']) ?></h2>
    </div>
    ...
    <form action="edit.php?id=<?= $id ?>" method="POST" style="padding: 1.5rem;">
        <div class="form-group">
            ...
            <select id="id_lapangan" name="id_lapangan" required>
                <?php foreach ($lapangan_list as $lapangan): ?>
                    <option value="<?= $lapangan['id_lapangan'] ?>" <?= ($lapangan['id_lapangan'] == $reservasi['id_lapangan']) ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            ...
            <input type="text" id="nama_pemesan" name="nama_pemesan" value="<?= htmlspecialchars($reservasi['nama_pemesan']) ?>" required>
        </div>
        <div class="form-group">
            <label for="no_telepon">No. Telepon</label>
            <input type="text" id="no_telepon" name="no_telepon" value="<?= htmlspecialchars($reservasi['no_telepon']) ?>" required>
        </div>
        <div class="form-group">
            <label for="tanggal_main">Tanggal Main</label>
            <input type="date" id="tanggal_main" name="tanggal_main" value="<?= htmlspecialchars($reservasi['tanggal_main']) ?>" required>
        </div>
        <div class="form-group">
            <label for="jam_mulai">Jam Mulai</label>
            <input type="time" id="jam_mulai" name="jam_mulai" value="<?= htmlspecialchars($reservasi['jam_mulai']) ?>" required>
        </div>
        <div class="form-group">
            <label for="jam_selesai">Jam Selesai</label>
            <input type="time" id="jam_selesai" name="jam_selesai" value="<?= htmlspecialchars($reservasi['jam_selesai']) ?>" required>
        </div>
        <div class="form-group">
            <label for="status_reservasi">Status</label>
            <select id="status_reservasi" name="status_reservasi" required>
                <option value="Aktif" <?= ($reservasi['status_reservasi'] == 'Aktif') ? 'selected' : '' ?>>Aktif</option>
